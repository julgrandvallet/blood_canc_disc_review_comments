...
# ─────────────────────────────────────────────
# 📈 New plot function (no coord_fixed)
# ─────────────────────────────────────────────
plot_lipid_zscore_avg <- function(df, compound_name) {
  lipid_df <- df %>% filter(compound_clean == compound_name)
  
  # Compute max range across both axes, with buffer
  max_val <- max(abs(c(lipid_df$Cells, lipid_df$Media)), na.rm = TRUE) * 1.1
...
for (lipid in target_lipids) {
    plot_obj <- plot_lipid_zscore_avg(zscored_avg_data_wide, lipid)
    ...
    file_safe_name <- gsub(" ", "_", lipid)
    filename <- paste0(output_dir, "/", file_safe_name, "_zScored_avg.pdf")
...
pdf("experiment_2_log2FC_heatmap.pdf", width = 8, height = 10)
